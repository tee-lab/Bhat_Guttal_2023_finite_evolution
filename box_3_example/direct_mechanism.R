...
# introduced in the main text (Box 3) of Bhat and Guttal 2024
# To illustrate the direct mechanism of noise-induced biasing
# Written by Shikhara Bhat
# JGU Mainz, Germany
# Date: Fri Jul  5 15:14:37 2024
###########################
...
#plot the case where direct mechanism does not operate
p <- ggplot(x=pvec) + geom_function(fun=no_freq_dep,args=list(a=1,c=10),linewidth=1,alpha=1,aes(color='b'))

#plot the case where direct mechanism operates
...
p <- p + scale_color_manual(values=c("#000000","#00aaff","#ff0000"),labels=unname(TeX(c("\\textbf{True neutral expectations (1/p(1-p))}","$|\\kappa / c|$ \\textbf{is small (Direct mechanism is weak/absent)}","$|\\kappa / c|$ \\textbf{is not small (Direct mechanism can operate)}"))))


#aesthetics
p <- p + theme_light() + ylab('Speed Density m(p)') + xlab("Trait frequency (p)")
...
#save to file
ggsave('./plots/direct_mechanism.svg',p,width=9.42,height=7.26,units='in')